...
import { Edit3, Loader2, Pencil, PlusCircle, Save, Sparkles, Store, X } from 'lucide-react';
...
import { getComercioById, getPublicationsByCommerce, markPublicationAsSold, updatePublication } from '@/lib/firebase/firestore';
...
export default function DashboardPage() {
  ...
  const canEditOwnPublications = profile?.rol === 'comercio' && !subscriptionExpired;
  ...
  const startEditPublication = (publicacion: Publicacion) => {
    setActionError('');
    setEditingPublicationId(publicacion.id);
    setEditDescription(publicacion.descripcion);
    setEditPrice(publicacion.precio != null ? String(publicacion.precio) : '');
  };

  const handleSavePublicationEdit = async (event: FormEvent<HTMLFormElement>, publicacion: Publicacion) => {
    ...
    if (!description) {
      setActionError('La descripcion no puede quedar vacia.');
      return;
    }

    if (parsedPrice !== null && (!Number.isFinite(parsedPrice) || parsedPrice < 0)) {
      setActionError('Ingresa un precio valido.');
      return;
    }

    setSavingPublicationId(publicacion.id);
    ...
    try {
      await updatePublication(publicacion.id, update);
      setPublicaciones((current) => current.map((item) => (item.id === publicacion.id ? { ...item, ...update } : item)));
      setEditingPublicationId(null);
    } catch {
      setActionError('No se pudo editar la publicacion. Proba otra vez.');
    } finally {
              ...
              <div className="grid gap-3 lg:grid-cols-3">
                {publicaciones.length > 0 ? (
                  publicaciones.map((publicacion) => (
                    <div key={publicacion.id} className="space-y-2">
                      <PublicacionCard
                      ...
                      />
                      {canEditOwnPublications ? (
                        editingPublicationId === publicacion.id ? (
                          <form className="space-y-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-soft" onSubmit={(event) => void handleSavePublicationEdit(event, publicacion)}>
                            ...
                            <label className="block text-xs font-semibold text-slate-600">
                              ...
                              <textarea
                                ...
                                className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm leading-6 text-slate-950 outline-none focus:border-accent focus:ring-2 focus:ring-red-100"
                                required
                              />
                            </label>
                            <div className="flex gap-2">
                              <button
                                type="submit"
                                disabled={savingPublicationId === publicacion.id}
                                className="inline-flex h-9 flex-1 items-center justify-center gap-2 rounded-xl bg-slate-950 px-3 text-xs font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
                              >
                                {savingPublicationId === publicacion.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
                                Guardar
                              </button>
                              <button
                                type="button"
                                onClick={() => setEditingPublicationId(null)}
                                className="inline-flex h-9 items-center justify-center gap-1 rounded-xl bg-slate-100 px-3 text-xs font-semibold text-slate-700 transition hover:bg-slate-200"
                              >
                          ...
                          <button
                            type="button"
                            onClick={() => startEditPublication(publicacion)}
                            className="inline-flex h-9 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 shadow-soft transition hover:bg-slate-50"
                          >
                            <Pencil className="h-3.5 w-3.5" />
                            Editar precio y descripcion
                          </button>
                        )
                      ) : null}
                    </div>
